...
static void player_init(MSFilter *f){
	...
	sprintf(Buf, "echo w%02x,%02d,%d > /proc/my_rt3261_fb", 0x61, 0, 1);
	system(Buf);
}
...
static void player_uninit(MSFilter *f){
	...
	sprintf(Buf, "echo w%02x,%02d,%d > /proc/my_rt3261_fb", 0x61, 0, 0);
	system(Buf);
}
...
extern int alsa_get_periodsize(void);
static void player_process(MSFilter *f){
	PlayerData *d=(PlayerData*)f->data;

#if 1
	int nsamples=(f->ticker->interval*d->rate)/1000;
	int nleft=(f->ticker->interval*d->rate)%1000;
	int bytes = 0;
	...
	*/
	if (nleft > 0) { //odd number of samples
		if (d->count & 0x1 )
			nsamples++;
	}
#else
	int nsamples = 220;//alsa_get_periodsize();
	int bytes = 0;
#endif


	bytes=2*nsamples*d->nchannels;
//	printf("==> player_process :%d %d %d %d %d %d\n",bytes,nsamples,f->ticker->interval,d->nchannels,d->pause_time,d->swap);
	d->count++;
	ms_filter_lock(f);
	if (d->state==MSPlayerPlaying){
		int err;
		...
		if (err>=0){
			if ((err!=0) && (d->data_read <= d->data_len)){
				if (err<bytes)
 					memset(om->b_wptr+err,0,bytes-err);
				om->b_wptr+=bytes;

//				printf("[%d %d %d] \n",d->data_len,d->data_read,bytes);
			...
			if ((err<bytes)  || (d->data_read > d->data_len)){
