...
#include "n32926-enc.h"

#define __N32916_PLATFORM___
#ifdef __N32916_PLATFORM___
...
#define STATIC_FRAME_RATE
#ifdef STATIC_FRAME_RATE
long long s_u64PrevTS = 0;
int s_fps = 30;
int s_u32VidInFrames = 0;
int s_u32VidInFirstTS = 0;
#endif
...
static int v4l_start(MSFilter *f, void *arg)
{
	...
	if (s->fd<0){
		...
		#if !defined(__N32916_PLATFORM___)
...
//#define PRINT_VMAP(vmap) do{ printf("[%s-%d] frame.%d - width.%d - height.%d - format.%d \n", \ 
//		__func__,__LINE__,vmap.frame,vmap.width,vmap.height,vmap.format); }while(0)

#define PRINT_VMAP(vmap)
...
//	printf("Trying to set capture size to %ix%i", vsize.width,vsize.height);
	err=ioctl(s->fd,VIDIOCSWIN,&win);
	if (err<0){
		ms_message("Could not set window size: %s",strerror(errno));
		return FALSE;
	}

	err=ioctl(s->fd, VIDIOCGWIN, &win);
	if (err<0){
		ms_message("Could not get window size: %s",strerror(errno));
		return FALSE;
	}
	...
	ms_message("try_size Capture size set to %ix%i", s->got_vsize.width,s->got_vsize.height);
	return TRUE;
}
...
#else
		fps = s->fps;
		ms_message("v4l_configure: set SW fps to be : %d", fps);
#endif	
	return 0;
}	
...
static mblk_t * v4l_grap_h264_mblk(V4lState *s,int syncframe){
DEBUG_ENTER();
	struct v4l2_buffer sV4L2Buff;
	sV4L2Buff.index = syncframe;
	ioctl(s->fd, VIDIOCGCAPTIME, &sV4L2Buff);

	unsigned int pu32BufPhyAddr = sV4L2Buff.m.userptr;	/* encode physical address */
	AVFrame pict;
	h264_phy_to_av_frame(s->enc_data,(unsigned char *)pu32BufPhyAddr,&pict);
	int h264_frame_szie = n32926_favc_encode(s->enc_data,&pict);
	
#ifdef STATIC_FRAME_RATE
	s_u64PrevTS = (unsigned long long)sV4L2Buff.timestamp.tv_sec * 1000000 + (uint64_t)sV4L2Buff.timestamp.tv_usec;
	if(s_u32VidInFrames == 0){
		s_u32VidInFirstTS = s_u64PrevTS;
	}
	s_u32VidInFrames ++;
	if(s_u32VidInFrames == 300){
		int nfps = s_u32VidInFrames / ((int)(s_u64PrevTS - s_u32VidInFirstTS) / 1000000);
//		printf("==> Vin frame rate %d fps \n", nfps); 
		s_fps = nfps;
		s_u32VidInFrames = 0;
		s->fps = nfps;
	}
	...
	return h264_frame_to_msgb((unsigned char * )(s->enc_data->out_h264_frame),h264_frame_szie);
}
...
static mblk_t * v4l_grab_image_mmap(V4lState *s){
	...
	if (err<0) {
		...
		usleep(2000); //10000=10ms and 2000=2ms
		return NULL;
	}
	...
	if (syncframe>=0){
		...
		int i32TryCnt = 0;
		while (ioctl(s->fd,VIDIOCSYNC,&syncframe) < 0 &&
			(errno == EAGAIN || errno == EINTR)){
				usleep(10000);

				i32TryCnt ++;
				if(i32TryCnt >= 100){
					printf("Error: V4L fail\n");
					return NULL;
			}
		}
		
	}else {
//		printf("==>Error return 222\n");
		return NULL;
	}

	return v4l_grap_h264_mblk(s,syncframe);
//	v4l_grap_h264_mblk(s,syncframe);
...
static void v4l_purge(V4lState *s){
	...
	for (i=s->frame_ind-jitter;i<s->frame_ind;++i){
		...
		if (err<0) {
			//ms_warning("v4l_purge: error in VIDIOCSYNC: %s.",strerror(errno));
		}
	}
}
...
static void *v4l_thread(void *ptr){
	...
	int err=-1;
	/*ms_message*/printf("v4l_thread starting\n");
...
/*
	...
	/*ms_message*/printf("v4l_thread exited.\n");
	ms_thread_exit(NULL);
...
static int n32926_find_h264_start_code(unsigned char *Buf,int size)
{
	if(!Buf || size < 4)
		return 0;
...
static int n32926_find_next_start_code(unsigned char *Buf,int size)
{
	if(!Buf || size < 4)
		return -1;
	...
	for(i = 0; i<= size-4 ; i++)
 	 	if(Buf[i] ==0 && Buf[i+1] ==0 && Buf[i+2] ==0 && Buf[i+3] ==1) 
			return i;//判断是否为0x00000001,如果是返回1

	return -1;
}

static MSQueue * n32926_put_q(void * buf,int length,MSQueue *q)
{
	mblk_t *om = allocb(length + 10, 0);
	memcpy(om->b_wptr, (void *)(buf + 4), length - 4);
	om->b_wptr += length - 4;
	ms_queue_put(q,om);
	return q;
}

static void x264_nals_to_msgb(mblk_t * im, MSQueue * nalus){

	unsigned char * buf = (unsigned char * )im->b_rptr;
	int frame_length = im->b_wptr - im->b_rptr;
	int per_frame_size = 0;
	
	while(n32926_find_h264_start_code(buf,frame_length))
	{
		switch(buf[4])
		{
			case 0x67:
				per_frame_size =4 + n32926_find_next_start_code(buf + 4,frame_length -4);
				n32926_put_q(buf,per_frame_size,nalus);
				break;
			case 0x68:
				per_frame_size =4 + n32926_find_next_start_code(buf + 4,frame_length -4);
				n32926_put_q(buf,per_frame_size,nalus);
...
//		printf("%x %x %x %x %x ",buf[4],buf[5],buf[6],buf[7],buf[8]);
		buf += per_frame_size;
		frame_length -= per_frame_size;
//		printf("[%d-%d] \n ",per_frame_size,frame_length);
	}
...
static void v4l_process(MSFilter * obj){
	...
	if (s->fd!=-1){
		...
		if (om!=NULL){
			...
			// timestamp=s->frame_count * (90000 / s->fps);
			x264_nals_to_msgb(om,&nalus);
			rfc3984_pack(s->enc_data->packer,&nalus,obj->outputs[0],timestamp);
