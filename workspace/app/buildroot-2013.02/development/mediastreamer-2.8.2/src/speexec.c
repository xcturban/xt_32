...
/*#define SPEEX_ECHO_CANCLE*/

#include "mediastreamer2/msfilter.h"
#include "mediastreamer2/msticker.h"

#ifdef SPEEX_ECHO_CANCLE
...
#else
#define EC_DUMP_PREFIX "/mnt/sdcard/tests"
#endif

//#define EC_DUMP 1

//-----------------------------------------------------
#define DELAY_MS_DEFAULT	150
#define SPON_ECHO_CANCEL

#ifdef SPON_ECHO_CANCEL
/*#include "echo_hisi2.h"*/
#include <EchoCancel.h>


//#define Q_BUF_ECHO
#ifdef Q_BUF_ECHO
#define N_BUFFER_SIZE		20
#define N_BYTES_PER_BLOCK	128
...
static unsigned char q_buf[N_BUFFER_SIZE*N_BYTES_PER_BLOCK];
static int q_front = 0, q_rear = 0;

int q_is_full()
{
	return (((q_front + 1) % N_BUFFER_SIZE) == q_rear)? Q_FULL : Q_OK;
}

int q_is_empty()
{
	return (q_front == q_rear)? Q_EMPTY : Q_OK;
}

int q_put(unsigned char * ref)
{
	if(Q_FULL != q_is_full())
	{
		memcpy(q_buf+q_front*N_BYTES_PER_BLOCK,ref,N_BYTES_PER_BLOCK);
		q_front = (q_front + 1) % N_BUFFER_SIZE;
		return Q_OK;
	}
	return Q_ERROR;
}
...
{
	char * buf = NULL;
	if(Q_EMPTY!= q_is_empty())
	{
		buf = (char *)(q_buf+q_rear*N_BYTES_PER_BLOCK);
		q_rear = (q_rear + 1) % N_BUFFER_SIZE;
	}
	return buf;
}

int q_loop_put(unsigned char * ref)
{
	if(Q_FULL == q_is_full())
		q_get();
...
#endif		//end of #ifdef Q_BUF_ECHO

#endif		//end of #ifdef SPON_ECHO_CANCEL
//-----------------------------------------------------
...
static void speex_ec_init(MSFilter *f){
...
#ifdef EC_DUMP
	{
		...
		ms_message("==>speex_ec_init : EC_DUMP %s\n",fname);
...
#ifdef SPON_ECHO_CANCEL
	EchoInit(s->framesize, 8000); //参数2要动态设置，16k对讲时设为16k，8k时设为8k
	//EchoCancel_Init();
	//EchoInit(s->framesize);
#endif
#ifdef SPEEX_ECHO_CANCLE
...
#define PRINT_MBLK(M,N) 	do{unsigned char* buf = M->b_rptr; int i = 0; \
		for(i=0;i<N;i++) \
				printf(" %02x",buf[i]); \
			printf(" \n"); \
		}while(0)
...
static void speex_ec_process(MSFilter *f){
	...
	if (f->inputs[0]!=NULL){
		if (s->echostarted){
			...
			ms_message("Getting reference signal but no echo to synchronize on.\n");
	...
	while (ms_bufferizer_read(&s->echo,echo,nbytes)>=nbytes){
		...
		if ((avail=ms_bufferizer_get_avail(&s->delayed_ref))<((s->nominal_ref_samples*2)+nbytes)){
			...
			if (!s->using_zeroes){
				ms_message("Not enough ref samples, using zeroes");
				s->using_zeroes=TRUE;
			}
		}else{
			if (s->using_zeroes){
...
#ifdef SPON_ECHO_CANCEL
		//start = get_now_ms();
		EchoCancel(oecho->b_wptr, echo, ref);
		
		//EchoCancel_Process(echo, ref, oecho->b_wptr);
		//end = get_now_ms();
		//printf("EchoCancel cost %lld ms\n", end - start);
#endif
