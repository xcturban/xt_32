...
extern void n32926_enc_fmt_set(int fmt);
extern void mx264_video_fmt_set(int fmt);
...
VideoStream *video_stream_new(int locport, bool_t use_ipv6){
	...
	switch(video_fmt_type)
	{
		case VIDEO_CIF_FMT:
			...
			break;
		case VIDEO_VGA_FMT:
			stream->sent_vsize.width=MS_VIDEO_SIZE_VGA_W;
			stream->sent_vsize.height=MS_VIDEO_SIZE_VGA_H;
			break;
		case VIDEO_D1_FMT:
			stream->sent_vsize.width=MS_VIDEO_SIZE_4CIF_W;
			stream->sent_vsize.height=MS_VIDEO_SIZE_4CIF_H;
			break;
		case VIDEO_720P_FMT:
			stream->sent_vsize.width=MS_VIDEO_SIZE_720P_W;
			stream->sent_vsize.height=MS_VIDEO_SIZE_720P_H;
			break;
		case VIDEO_1080P_FMT:
			stream->sent_vsize.width=MS_VIDEO_SIZE_1080P_W;
			stream->sent_vsize.height=MS_VIDEO_SIZE_1080P_H;
			break;
		default:
			stream->sent_vsize.width=MS_VIDEO_SIZE_CIF_W;
			stream->sent_vsize.height=MS_VIDEO_SIZE_CIF_H;
...
static void configure_video_source(VideoStream *stream){
	...
	ms_message("Setting sent vsize=%ix%i, fps=%f\n",vsize.width,vsize.height,fps);
...
void video_fmt_set(int fmt)
{
	video_fmt_type = fmt;
	mx264_video_fmt_set(fmt);
	n32926_enc_fmt_set(fmt);
}
...
int video_stream_start (VideoStream *stream, RtpProfile *profile, const char *remip, int remport,
	...
	// 构造一个视频数据发送方的网络地址，用于过滤数据包，因为事先不知道发送方的端口，所以这里只用ip来过滤，
	// 端口任意指定。
	if (remport == 0 && stream->dir == VideoStreamRecvOnly) {
		int err;
		struct sockaddr_in *sin = &stream->session->rtp.rem_addr;

		memset(sin, 0, sizeof(struct sockaddr_in));
		sin->sin_family = AF_INET;
		if (inet_pton(AF_INET, remip, &(sin->sin_addr)) <= 0)
		{
	...
	if (stream->dir==VideoStreamSendRecv || stream->dir==VideoStreamSendOnly){
		...
		ms_filter_link (stream->tee, 0, stream->rtpsend, 0);
		#endif
	...
	if (stream->dir==VideoStreamSendRecv || stream->dir==VideoStreamRecvOnly){
		...
		ms_connection_helper_link (&ch,stream->decoder,0,-1);
		if (stream->tee2){
...
video_stream_stop (VideoStream * stream)
{
	...
	if (stream->ticker){
		...
		if (stream->source){
			...
			if (stream->rtpsend)
				ms_filter_unlink (stream->tee, 0, stream->rtpsend, 0);
		#endif
			if (stream->output2){
