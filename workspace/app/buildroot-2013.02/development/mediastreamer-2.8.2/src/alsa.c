...
#define ALSA_PERIOD_SIZE 220//220//512//220
...
static int exact_periodsize = ALSA_PERIOD_SIZE;
int alsa_get_periodsize(void)
{
    return exact_periodsize;
}
...
static int alsa_set_params(snd_pcm_t *pcm_handle, int rw, int bits, int stereo, int rate)
{
    ...
    if (dir != 0)
    {
        /*ms_warning*/printf("alsa_set_params: The period size %d is not supported by your hardware.\n "
                             "==> Using %d instead.", periodsize, (int)exact_ulvalue);
    }
    /*ms_warning*/printf("alsa_set_params: periodsize:%d Using %d", periodsize, (int)exact_ulvalue);
...
static void alsa_fill_w (snd_pcm_t *pcm_handle)
{
    ...
    void *buffer;
    buffer = alloca (ALSA_PERIODS * ALSA_PERIOD_SIZE);
    memset (buffer, 0, ALSA_PERIODS * ALSA_PERIOD_SIZE);

    /* write data */
    snd_pcm_writei(pcm_handle, buffer, ALSA_PERIODS * ALSA_PERIOD_SIZE / 2);
#endif
}
#endif

snd_pcm_t * alsa_open_r_block(const char *pcmdev,int bits,int stereo,int rate)
{
    snd_pcm_t *pcm_handle;
    int err;

    /*ms_message*/
    printf("alsa_open_r: opening %s at %iHz, bits=%i, stereo=%i",pcmdev,rate,bits,stereo);
...
snd_pcm_t * alsa_open_w_block(const char *pcmdev,int bits,int stereo,int rate)
{
    snd_pcm_t *pcm_handle;

    /*ms_message*/
    printf("alsa_open_w_block: opening %s at %iHz, bits=%i, stereo=%i\n",pcmdev,rate,bits,stereo);

#ifndef THREADED_VERSION
    if (snd_pcm_open(&pcm_handle, pcmdev,SND_PCM_STREAM_PLAYBACK,0) < 0)
    {
        perror("snd_pcm_open 222\n");
        ms_warning("alsa_open_w_block: Error opening PCM device %s",pcmdev );
        return NULL;
    }
#else
    if (snd_pcm_open(&pcm_handle, pcmdev,SND_PCM_STREAM_PLAYBACK,0) < 0)
...
static void * alsa_write_thread(void *p)
{
    ...
    printf("alsa_write_thread enter\n");
...
void alsa_read_process(MSFilter *obj)
{
    ...
    if (ad->handle==NULL)
    {
        ad->handle=alsa_open_r(/*ad->pcmdev*/"default:1",16,ad->nchannels==2,ad->rate);
        if (ad->handle)
        {
            ad->read_samples=0;
...
#ifdef THREADED_VERSION
static void *alsa_read_thread(void *p)
{
    AlsaWriteData *ad = (AlsaWriteData*)p;
    int samples = (ALSA_PERIOD_SIZE * ad->rate) / 8000;
    int size = samples * 2 * ad->nchannels;
    unsigned char *bytes = malloc(size);

    if (bytes == NULL)
    {
        printf("alsa_read_thread malloc error\n");
        return NULL;
    }
    ...
    printf("alsa_read_thread enter\n");

    while (ad->write_started)
    {
        ms_mutex_lock(&ad->mutex);
        while (ms_bufferizer_get_avail(ad->bufferizer) >= size)
        {
            ms_bufferizer_read(ad->bufferizer, bytes, size);
            alsa_write(ad->handle, bytes, samples);
        }
        ms_mutex_unlock(&ad->mutex);
    ...
    printf("alsa_read_thread exit\n");


    if (ad->handle != NULL)
        snd_pcm_close(ad->handle);
    ad->handle=NULL;
    free(bytes);
    return NULL;
}

static void alsa_start_w(AlsaWriteData *d)
{
    if (d->write_started == FALSE)
    {
        d->write_started = TRUE;
        ms_thread_create(&d->thread, NULL, alsa_read_thread, d);
    }
    else
        d->write_started = TRUE;
}

static void alsa_stop_w(AlsaWriteData *d)
{
...
void alsa_write_preprocess(MSFilter *obj)
{
#ifdef THREADED_VERSION
...
void alsa_write_process(MSFilter *obj)
{
    ...
    if (ad->handle==NULL)
    {
        ad->handle=alsa_open_w(/*ad->pcmdev*/"default", 16, ad->nchannels==2,ad->rate);
#ifdef EPIPE_BUGFIX
...
void alsa_write_process(MSFilter *obj)
{
    ...
    ms_mutex_lock(&ad->mutex);
    while ( (im = ms_queue_get(obj->inputs[0])) != NULL)
        ms_bufferizer_put(ad->bufferizer, im);
    ms_mutex_unlock(&ad->mutex);
...
MSFilterDesc alsa_write_desc=
{
    ...
    .preprocess=alsa_write_preprocess,
