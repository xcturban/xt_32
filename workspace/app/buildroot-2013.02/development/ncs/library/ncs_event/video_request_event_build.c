...
static ncs_event_data_t *ncs_video_request_event_data_pack(
		video_class_t class,
		video_fmt_t fmt,
		uint8_t board_id,
		char *ip,
		...
		int *evt_data_len)
{
	...
	*evt_data_len = len;
	...
	evt_data->msg = 0x5b;
	evt_data->param1 = (uint8_t)class;
	if (board_id > 0)
		evt_data->param2 = board_id - 1; /* 面板号从0开始 */
	else
		evt_data->param2 = 0xfe;
...
ncs_event_t *ncs_video_request_event_build(struct sockaddr *target_sockaddr, video_class_t class, video_fmt_t fmt, uint8_t board_id, char *ip, uint16_t port)
{
	...
	evt_data = ncs_video_request_event_data_pack(class, fmt, board_id, ip, port, &evt_data_len);
	if (evt_data == NULL) {
