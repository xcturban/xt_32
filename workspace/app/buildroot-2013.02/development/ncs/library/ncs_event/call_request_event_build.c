...
static ncs_event_data_t *ncs_call_request_event_data_pack(
		uint16_t target_id, 
		uint8_t board_id,
		int samplerate,
...
ncs_event_t *ncs_call_request_event_build(struct sockaddr *target_sockaddr, uint16_t target_id, uint8_t board_id, int samplerate, ncs_audio_encode_t audio_encode)
{
	...
	evt_data = ncs_call_request_event_data_pack(target_id, board_id, samplerate, audio_encode, &evt_data_len);
	if (evt_data == NULL) {
