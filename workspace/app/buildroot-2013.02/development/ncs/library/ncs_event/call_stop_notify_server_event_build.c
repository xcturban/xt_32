...
static ncs_event_data_t *ncs_call_stop_notify_server_event_data_pack(
		uint16_t target_id,
		uint8_t board_id,
		int *evt_data_len)
{
...
ncs_event_t *ncs_call_stop_notify_server_event_build(uint16_t target_id, uint8_t board_id)
{
	...
	evt_data = ncs_call_stop_notify_server_event_data_pack(target_id, board_id, &evt_data_len);
	if (evt_data == NULL) {
