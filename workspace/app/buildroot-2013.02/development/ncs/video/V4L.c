...
)
{
	...
	if (ioctl(i32VidFD, VIDIOCGMBUF, &s_sVidData.sVidMBufs) < 0) {
		...
		s_sVidData.sVidMMap.format = VIDEO_PALETTE_YUV420P;//VIDEO_PALETTE_YUV422P;//VIDEO_PALETTE_YUV420P;
...
)
{
	...
	*pu32BufPhyAddr = sV4L2Buff.m.userptr;	/* encode physical address */
	...
	//printf("%s-%d:%"PRId64" us\n", __func__, __LINE__, (*pu64TimeStamp - s_u64PrevTS));
...
void FinializeV4LDevice()
{
	...
	close(s_sVidData.i32VidFD);
    printf("s_sVidData.i32VidFD close \n");
...
{
	//E_IMAGE_RESOL ePreviewImgResol = eIMAGE_CIF;
	//E_IMAGE_RESOL eEncodeImgResol = eIMAGE_CIF;

	int32_t i32Ret = 0;

	//uint32_t u32EncodeImgWidth=0, u32EncodeImgHeight=0;
	//uint32_t u32PreviewImgWidth=0, u32PreviewImgHeight=0;
	//video_image_resolution_get(ePreviewImgResol,&u32PreviewImgWidth,&u32PreviewImgHeight);
	//video_image_resolution_get(eEncodeImgResol,&u32EncodeImgWidth,&u32EncodeImgHeight);
	if(InitV4LDevice(352, 288) != ERR_V4L_SUCCESS){
		i32Ret = -1;
		DEBUG_PRINT("Init V4L device fail\n");
		goto exit_prog;
	}
	DEBUG_PRINT("Init V4L device Success\n");
	//Planar H264_2D
	StopV4LCapture();
	SetV4LEncode(0, 352, 288, VIDEO_PALETTE_YUV420P_MACRO);

	//SetV4LViewWindow(u32PreviewImgWidth, u32PreviewImgHeight, u32PreviewImgWidth, u32PreviewImgHeight);
	StartV4LCapture();
	DEBUG_PRINT("Start capturing\n");
