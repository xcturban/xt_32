...
#ifdef __NAS8532_RECEIVED_CALL_H__
...
int received_call_add(int target_id, int board_id)
{
    ...
    cfg = lp_config_new(INI_RECEIVED_CALL_FILE);
    if (cfg == NULL)
    {
    ...
    total = lp_config_get_int(cfg, "received_call_total", "total", 0);
    if (total < 0)
    {
        lp_config_set_int(cfg, "received_call_total", "total", 0);
        total = 0;
    }

    if (total >= MAX_RECEIVED_CALL)
    {
        lp_config_destroy(cfg);
        received_call_del(1);
        received_call_add(target_id, board_id);
        return 0;
    }
    ...
    sprintf(section, "received_call_%d", total + 1);

    lp_config_set_int(cfg, "received_call_total", "total", total + 1);
...
int received_call_del(int idx)
{
    ...
    sprintf(section, "received_call_%d", idx);
    ...
    for (i = idx + 1; i <= total; i++)
    {
        ...
        sprintf(section, "received_call_%d", i);
        ...
        sprintf(section, "received_call_%d", i - 1);
        lp_config_set_int(cfg, section, "target_id", target_id);
        lp_config_set_int(cfg, section, "board_id", board_id);
...
{
    char cl[128] = {0};
    sprintf(cl, "echo > %s", INI_RECEIVED_CALL_FILE);
    system(cl);
