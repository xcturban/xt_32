...
#ifdef __NAS8532_MISSED_CALL_H__
...
int missed_call_add(int target_id, int board_id)
{
	...
	if (total < 0) {
		lp_config_set_int(cfg, "missed_call_total", "total", 0);
		total = 0;
	}

	if (total >= MAX_MISSED_CALL) {
		lp_config_destroy(cfg);
		missed_call_del(1);
		missed_call_add(target_id, board_id);
		return 0;
	}
	...
	sprintf(section, "missed_call_%d", total + 1);

	lp_config_set_int(cfg, "missed_call_total", "total", total + 1);
...
int missed_call_del(int idx)
{
	...
	sprintf(section, "missed_call_%d", idx);
	...
	for (i = idx + 1; i <= total; i++) {
		...
		sprintf(section, "missed_call_%d", i);
		...
		sprintf(section, "missed_call_%d", i - 1);
		lp_config_set_int(cfg, section, "target_id", target_id);
		lp_config_set_int(cfg, section, "board_id", board_id);
...
{
	char cl[128] = {0};
	sprintf(cl, "echo > %s", INI_MISSED_CALL_FILE);
	system(cl);
